// Updated Arta Nusa Shop with cart improvements, variant support, clear cart, brand logo, and country-aware pricing
      ...
      {selected && selectedVariant && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setSelected(null)}>
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative" onClick={(e) => e.stopPropagation()}>
            ...
            <select onChange={(e) => setSelectedVariant(selected.variants.find(v => v.id === e.target.value))} className="w-full border mt-2 mb-4 p-2">
              {selected.variants.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
            </select>
